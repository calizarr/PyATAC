...
def options():
    ...
    parser.add_argument("-i", "--indices", help="Directory of bowtie indices (default: Brachy)")
...
def main():
    ...
    cmd = "bowtie --chunkmbs 256 -p {0} -S -m 1 -X 2000 -t {1} {2} | samtools view - -bS | samtools sort - -o {3} -O bam -T {2}.pre" \
          .format(args.nThreads, indices, args.input_file, output_file)
    print("Running cmd: ")
    print(cmd)
    subprocess.call(cmd, shell=True)
    # Only mapped reads.
    cmd = "samtools view -b -F {0} > {0}.sorted.bam".format(output_file)
    print("Running cmd: ")
    print(cmd)
    subprocess.call(cmd, shell=True)
